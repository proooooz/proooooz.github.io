...
2. [**_REACHES_ 中国历史气候数据库**](https://reaches.rcec.sinica.edu.tw/index.html)

   <details class="resource-details">
     <summary>详细介绍</summary>
     <p><em>REACHES</em> 的全称为 <em>Reconstructed East Asian Climate Historical Encoded Series</em>，是一套将中国历史文献中的气候、气象、灾害及其社会影响记录转化为可检索、可计算数据的历史气候数据库。该项目主要由中央研究院环境变迁研究中心等机构的研究团队建设，其核心资料来源是《中国三千年气象记录总集》，并综合正史、实录、地方志以及少量其他历史文献中的相关记载。数据库的目标不是简单保存历史文献原文，而是把“某时、某地发生了何种天气或气候事件”的文字描述编码为结构化记录，使研究者能够利用现代统计与空间分析方法研究中国长期气候变化及其经济社会后果。</p>
     <p>REACHES 官方平台目前提供明清时期 1368—1911 年完整记录的查询与下载入口。数据库建设论文重点介绍了 1644—1795 年样本的数字化过程：该阶段共包含 36,123 条原始文献记录，涉及 1,435 个地理地点；在把一条文献中不同时间或不同属性的事件拆分后，形成 49,714 条事件层面的二级记录。原始资料中的农历日期和历史地名被进一步转换为公历时间与经纬度信息，从而使历史记录同时具有可比较的时间坐标和空间坐标。</p>
     <p>在数据生成过程中，研究团队首先对《中国三千年气象记录总集》中的条目进行扫描、文字识别和人工校对，再将每条记录拆解为<strong>时间、地点、事件、编码者和资料来源</strong>五个基本维度。REACHES 区分原始文献层面的 Level-1（L1）记录与事件层面的 Level-2（L2）记录：如果一条原始文献同时记载了不同月份、不同地区或多种气候现象，数据库会将其拆分为多条 L2 记录，同时保留能够追溯原始条目的记录编号。这种结构既便于统计单一气候事件，也使研究者可以返回原始资料核查具体语境。</p>
     <p>数据库采用分层事件分类体系，将历史记载依次划分为领域、主要类别、子类别和更细的事件属性。其内容包括降水、气温、干旱、洪涝、降雪、霜冻、冰雹、雷电、大风、台风等气象现象，也涵盖农作物生长与收成、饥荒、疫病、蝗灾及其他社会经济后果。官方查询数据中的常见字段包括事件代码、地点、省份、农历起止时间、公历起止时间、主要类别、子类别以及英文地名等；记录还可关联经度、纬度和海拔，从而支持按年份、季节、月份、地点或事件类型进行筛选和空间匹配。</p>
     <p>在获取方式上，研究者可以通过 <a href="https://reaches.rcec.sinica.edu.tw/index.html">REACHES 查询平台</a>按时间、地点和事件类别检索，并下载全部数据、特定类别数据或元数据。美国国家海洋和大气管理局的 <a href="https://www.ncei.noaa.gov/access/paleo-search/study/23410">NOAA/NCEI 归档页面</a>同时提供 REACHES v3.1 数据文件、气象与灾害事件代码表、说明文件和 Excel 汇总文件，便于程序化整理与长期保存。使用数据时应同时保存下载版本、访问日期、事件代码表和元数据，并按照 <a href="https://doi.org/10.1038/sdata.2018.288">Wang 等（2018）的数据库建设论文</a>进行引用。</p>
     <p>在历史经济学研究中，REACHES 可以用于构建地区—年份或地区—季节层面的干旱、洪涝、极端寒冷、异常高温、降雪、台风和蝗灾等冲击指标，也可以进一步重建温度、湿润程度和降水指数。将其与 CHGIS 的历史地名及行政区划数据匹配后，可把事件记录聚合到历史县、府或省级单位，再与人口、粮价、税收、科举、战争、叛乱、移民、农业产出和地方财政等资料连接，用于研究气候冲击对农业生产、人口变化、社会冲突、国家能力、市场整合和长期经济发展的影响。</p>
     <p>使用 REACHES 时需要特别注意历史文献记录的<strong>选择性与不均衡性</strong>。传统史料更倾向记载显著、异常或造成社会后果的事件，未出现记录并不等于当地没有发生相应气候现象；记录密度在时间和空间上也并不均匀，黄河与长江流域及传统政治经济中心的资料较丰富，而边疆和人口稀疏地区相对较少。因此，REACHES 更适合识别异常气候事件及其相对变化，而不能直接等同于现代气象站的连续测量值。实证分析中应控制地区史料丰富程度和年代趋势，检验不同事件定义与聚合口径的稳健性，并避免把史料保存或地方志编纂差异误判为真实气候变化。</p>
   </details>
